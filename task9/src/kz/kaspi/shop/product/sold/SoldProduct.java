...
public class SoldProduct implements Serializable {
    ...
    public boolean willExpireWithin(int days) {
        return expireDate.isAfter(LocalDate.now()) && expireDate.isBefore(LocalDate.now().plusDays(days));
    }

    public boolean soldLastDays(int days) {
        return soldDate.isAfter(LocalDate.now().minusDays(days)) && soldDate.isBefore(LocalDate.now().plusDays(1));
    }
