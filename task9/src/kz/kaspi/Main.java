...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        if (shop.isEmpty())                         // If shop is empty, fill with buyers
            getBuyers().forEach(shop::addBuyer);

        shop.processAll();                          // Process all buyers in shop

        if (!shop.save())
            Logger.getInstance().error("Cannot save shop");
    }
